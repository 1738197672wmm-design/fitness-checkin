      ...
      <div className='container'>
        ...
        <BorderGlow borderRadius={16} glowRadius={35} edgeSensitivity={25} glowIntensity={0.7} colors={['#c084fc', '#f472b6', '#38bdf8']} backgroundColor="transparent">
          <div className='checkin-form-card'>
            <h3 className='cf-title'>? ??</h3>
            {step === 1 && (
              <div className='cf-step'>
                <div className='cf-step-label'>?? 1/3 ? ??????</div>
            ...
            {step === 2 && (
              <div className='cf-step'>
                <div className='cf-step-label'>?? 2/3 ? ????</div>
                ...
                <form onSubmit={submitDuration} className='cf-number-form'>
                  <div className='cf-number-input-wrap'>
                    <input
                      ...
                      autoFocus
                    />
                    <span className='cf-number-unit'>??</span>
                  </div>
                  <div className='cf-step-actions'>
                    <button type='button' onClick={resetForm} className='cf-btn-back'>??</button>
                    <button type='submit' className='cf-btn-next' disabled={!duration || Number(duration) <= 0}>???</button>
                  </div>
                </form>
              </div>
            )}
            {step === 3 && (
              <div className='cf-step'>
                <div className='cf-step-label'>?? 3/3 ? ????</div>
                <div className='cf-step-badge'>
                  <span className='cf-badge-icon'>{typeIcon}</span>
                  <span>{typeLabel} ? {duration} ??</span>
                </div>
                <form onSubmit={submitCalories} className='cf-number-form'>
                  <div className='cf-number-input-wrap'>
                    <input
                      ...
                      autoFocus
                    />
                    <span className='cf-number-unit'>?? (kcal)</span>
                  </div>
                  <div className='cf-step-actions'>
                    <button type='button' onClick={() => setStep(2)} className='cf-btn-back'>???</button>
          ...
          <div className='ws-bars'>
            <div className='ws-bar-item'>
              <div className='ws-bar-label'>??</div>
        ...
        <div className='dashboard-tabs'>
          <button className={'tab-btn' + (activeTab === 'checkin' ? ' active' : '')} onClick={() => setActiveTab('checkin')}>????</button>
          <button className={'tab-btn' + (activeTab === 'feed' ? ' active' : '')} onClick={() => setActiveTab('feed')}>????</button>
        </div>
        {activeTab === 'checkin' ? (
